...
export class CustomDrawingSheet extends DrawingConfig {
  ...
  async _prepareContext(options) {
    ...
    context.showPinSelector =
      game.settings.get(MODULE_ID, 'pinColor') !== 'none';
  ...
  _onRender(context, options) {
    ...
    tabBtns.forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        this.element
          .querySelectorAll('.ib-tab-panel')
          .forEach((p) => p.classList.remove('active'));
        btn.classList.add('active');
        this.element
          .querySelector(`.ib-tab-panel[data-panel="${btn.dataset.tab}"]`)
          ?.classList.add('active');
      });
    });

    // ---- Film grain intensity — live label ----
    const grainRange = this.element.querySelector(
      "[name='videoEffects.filmGrainIntensity']",
    );
    ...
    const previewVideoBtn = this.element.querySelector('.ib-preview-video-btn');
    if (previewVideoBtn) {
    ...
    if (resetBtn) {
      resetBtn.addEventListener('click', () => {
        const formatKey =
          this.element.querySelector("[name='videoFormat']")?.value ?? 'crt';
    ...
    const tsSize = this.element.querySelector(
      "[name='videoEffects.timestampFontSize']",
    );
    const tsClr = this.element.querySelector(
      "[name='videoEffects.timestampColor']",
    );
    ...
    if (tsSize)
      tsSize.addEventListener('input', () => {
        if (szVal) szVal.textContent = `${tsSize.value}px`;
        getPlayer()?.updateTimestampStyle({ fontSize: parseInt(tsSize.value) });
      });
    if (tsClr)
      tsClr.addEventListener('input', () => {
        getPlayer()?.updateTimestampStyle({ color: tsClr.value });
      });

    // ---- Debounced full-effects preview (any change in effects tab → applyEffects) ----
    const effectsPanel = this.element.querySelector(
      ".ib-tab-panel[data-panel='effects']",
    );
    if (effectsPanel) {
      let effectsDebounce = null;
      const scheduleApply = () => {
        clearTimeout(effectsDebounce);
        effectsDebounce = setTimeout(() => {
          getPlayer()?.applyEffects(this._readEffectsFromForm());
        }, 500);
      };
      // Sliders/number/text inputs bubble `input` events normally
      effectsPanel.addEventListener('input', scheduleApply);
      // Foundry AppV2 intercepts bubbled `change` events in capture phase on the form,
      // so attach directly to checkboxes/radios (target-phase listeners always fire)
      effectsPanel
    ...
    if (dropZone) {
      ...
      dropZone.addEventListener('drop', async (ev) => {
        ...
        try {
          ...
          console.error(
            'Investigation Board: Failed to process drop in sheet',
            err,
          );
        }
      });
    ...
    const audioPickerButton = this.element.querySelector(
      '.audio-picker-button',
    );
    ...
    if (previewAudioBtn) {
      previewAudioBtn.addEventListener('click', async (ev) => {
        ...
        const audioPath = this.element.querySelector(
          "input[name='audioPath']",
        )?.value;
        if (audioPath) {
          this.previewSound = await game.audio.play(audioPath, { volume: 0.8 });

          // Apply tape effect if enabled in the form
          const effectEnabled = this.element.querySelector(
            "input[name='audioEffectEnabled']",
          )?.checked;
    ...
    const mediaRadios = this.element.querySelectorAll(
      "input[name='mediaMode']",
    );
    ...
    mediaRadios.forEach((radio) => {
      radio.addEventListener('change', async () => {
        ...
        await collaborativeUpdate(this.document.id, {
          ...
          [`flags.${MODULE_ID}.image`]: isVideo
            ? getRandomVideoImage()
            : getRandomCassetteImage(),
          [`flags.${MODULE_ID}.pinColor`]: isVideo ? 'none' : '',
    ...
    // Video file picker
    const videoPickerButton = this.element.querySelector(
      '.video-picker-button',
    );
  ...
  /** Read all current videoEffects values from the open form. */
  _readEffectsFromForm() {
    const g = (name) =>
      this.element.querySelector(`[name="videoEffects.${name}"]`);
    return {
      rollingShutter: g('rollingShutter')?.checked ?? false,
